...
router.post('/whatsapp/lifecycle-sync', syncLifecycle);
router.post('/whatsapp/repair-all', repairAllWaSessions);
    ...
    if (err) {
      ...
      if (err instanceof multer.MulterError) {
        if (err.code === 'LIMIT_FILE_SIZE') {
          res.status(400).json({ success: false, error: 'Ukuran file maksimal 10MB' });
          return;
        }
