...
export async function POST(
  ...
  try {
    ...
    const { wa_user_id, channel, channel_identifier, session_id, sessionId } = body as {
      wa_user_id?: string;
    ...
    const response = await fetch(`${CASE_SERVICE_URL}/laporan/${id}/check`, {
      method: "POST",
      headers: {
    ...
    try {
      ...
      if (!response.ok) {
        return NextResponse.json(
          { error: responseText.substring(0, 200) || "Gagal memuat status laporan" },
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: result?.message || result?.error || "Gagal memuat status laporan" },
        { status: response.status }
      );
    }
    ...
    console.error("Public complaint check error:", error);
