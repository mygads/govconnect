...
// GET - List semua kategori nomor penting
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const categories = await prisma.important_number_categories.findMany({
...
// POST - Buat kategori nomor penting baru
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const lastCategory = await prisma.important_number_categories.findFirst({
      where: { village_id: villageId },
      orderBy: { order: 'desc' }
    });

    const category = await prisma.important_number_categories.create({
      data: {
    ...
    console.error('Create important number category error:', error);
    return NextResponse.json(
