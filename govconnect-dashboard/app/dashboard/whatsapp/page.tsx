...
export default function WhatsAppPage() {
  ...
  const [pairPhoneNumber, setPairPhoneNumber] = useState<string>("")
  ...
  // Fetch session status (sync with WA server)
  const fetchSessionStatus = useCallback(async (showToast = false) => {
    try {
  ...
  const handleSync = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Status WhatsApp berhasil disinkronkan dengan server.",
      })
    } catch (error) {
      toast({
        title: "Error",
        description: "Gagal menyinkronkan status WhatsApp.",
        variant: "destructive",
      })
    } finally {
      setIsSyncing(false)
    }
  }, [fetchSessionStatus, fetchSessionSettings, toast])

  // Initial data load - sync with WA server on page load
  useEffect(() => {
    ...
    loadData()
  }, [fetchSessionStatus, fetchSessionSettings])
